...
import { Sparkles, Send, MoreVertical, Search, Edit, X, User, Trash2, Loader, ChevronRight } from 'lucide-react';
...
function NewSynapseModal({ onClose, onSelectContact }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl p-8 shadow-2xl animate-fade-in flex flex-col gap-6 min-h-[400px]" style={glassStyle}>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-lg">New Synapse</h3>
            <p className="text-xs text-sc-muted mt-0.5">Search the neural network</p>
          </div>
          <button onClick={onClose} className="text-sc-muted hover:text-white transition p-1">
            <X size={18} />
          </button>
        </div>

        <div className="relative flex items-center">
          <Search className="absolute left-4 text-sc-muted pointer-events-none" size={16} />
          <input
            autoFocus
            type="text"
            placeholder="Enter username or display name..."
            className="sc-input pl-10 text-sm"
            value={query}
        ...
        <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
          {searching ? (
            <div className="text-center py-6 text-sc-muted animate-pulse text-sm">Scanning network...</div>
          ) : results.length === 0 && query ? (
            <div className="text-center py-6 text-sc-muted text-sm">No users found on this frequency.</div>
          ) : (
            results.map(u => (
              <button
                key={u.id}
                onClick={() => { onSelectContact({ _id: u.id, displayName: u.displayName, username: u.username, avatarUrl: u.avatarUrl, lastMessage: 'New connection...', timestamp: new Date().toISOString() }); onClose(); }}
              ...
              >
                <div className="w-10 h-10 rounded-full bg-sc-hover overflow-hidden flex-shrink-0 flex items-center justify-center font-bold">
                  {u.avatarUrl ? <img src={u.avatarUrl} alt={u.username} className="w-full h-full object-cover" /> : u.displayName?.[0]}
                </div>
                <div>
...
export default function Messages() {
    ...
    const fetchData = async () => {
      try {
        ...
        allMessages.forEach(msg => {
          const otherUser = msg.senderId?._id === currentUser.id ? msg.receiverId : msg.senderId;
          if (!otherUser) return;
        ...
        const contactList = Array.from(contactMap.values()).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
        setContacts(contactList);
        if (contactList.length > 0) setActiveContact(prev => prev || contactList[0]);
      } catch (err) {
    ...
    const handleNewMessage = ({ message }) => {
      ...
      if (senderId && senderId !== currentUser.id) {
        setContacts(prev => prev.map(c =>
          (c._id === senderId) ? { ...c, lastMessage: message.content, timestamp: message.createdAt } : c
        ));
      }
    };
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m._id === optimisticId ? res.data.data.message : m));
      // Update contact last message
      setContacts(prev => prev.map(c =>
        c._id === activeContact._id ? { ...c, lastMessage: optimistic.content, timestamp: new Date().toISOString() } : c
      ));
    } catch (err) {
  ...
  const handlePurge = async () => {
    ...
    try {
      ...
      setMessages(prev => prev.filter(m =>
        !((m.senderId?._id === currentUser.id && m.receiverId?._id === activeContact._id) ||
          (m.receiverId?._id === currentUser.id && m.senderId?._id === activeContact._id))
      ));
  ...
  return (
    <>
      {showNewSynapse && (
        <NewSynapseModal onClose={() => setShowNewSynapse(false)} onSelectContact={handleSelectContact} />
      )}
      ...
      <div className="flex h-full w-full overflow-hidden">
        ...
        <div className="w-[300px] flex-shrink-0 bg-[#081329]/80 flex flex-col border-r border-white/5">
          <div className="p-5 flex justify-between items-center border-b border-white/5 bg-[#142449]/30 backdrop-blur-md">
            <h2 className="text-lg font-extrabold tracking-tight">Active Synapses</h2>
            <button
          ...
          <div className="px-4 py-3">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-sc-muted" size={15} />
              <input
                type="text"
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              <div className="flex justify-center items-center h-32 animate-pulse text-sc-muted text-xs uppercase tracking-widest gap-2">
                <Sparkles className="text-sc-accent-light" size={14} /> Syncing...
              </div>
            ) : contacts.length === 0 ? (
              <div className="text-center p-6 text-sc-muted text-xs mt-4">
                No connections yet. Click <Edit size={12} className="inline" /> to start a new synapse!
              </div>
            ) : (
              contacts.map(c => (
                <div
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <h4 className="font-bold text-sm truncate">{c.displayName}</h4>
                      <span className="text-[10px] text-sc-muted whitespace-nowrap ml-2">
                        {new Date(c.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    <p className="text-xs text-sc-muted truncate">{c.lastMessage}</p>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="flex-1 flex flex-col bg-[#060e20]/40 overflow-hidden">
          {activeContact ? (
            <div key={activeContact._id} className="flex flex-col h-full animate-fade-in">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-accent flex items-center justify-center font-bold overflow-hidden">
                    {activeContact.avatarUrl
                      ? <img src={activeContact.avatarUrl} className="w-full h-full object-cover" alt="" />
                      : activeContact.displayName?.[0]?.toUpperCase()}
                  </div>
                  <div>
                    <h3 className="font-extrabold leading-none">{activeContact.displayName}</h3>
                ...
                <div className="flex items-center gap-2 relative">
                  ...
                  {showMenu && (
                    <ContactMenu
                      ...
                      onPurge={handlePurge}
                      onViewProfile={() => { setShowMenu(false); navigate(`/profile/${activeContact._id}`); }}
                    />
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-4">
                ...
                {currentChatMessages.map(msg => {
                  ...
                  return (
                    <div key={msg._id} className={`flex flex-col max-w-[75%] ${isMe ? 'self-end' : 'self-start'} ${msg.optimistic ? 'opacity-60' : ''}`}>
                      <div className={`px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap rounded-2xl ${isMe ? 'bg-gradient-to-br from-[#7c3aed] to-[#a855f7] text-white rounded-br-sm shadow-[0_4px_15px_rgba(124,58,237,0.3)]' : 'bg-[#142449] text-sc-text rounded-bl-sm shadow-[0_4px_15px_rgba(0,0,0,0.2)]'}`}>
                        {msg.content}
                      </div>
                      <span className={`text-[10px] mt-1 text-sc-muted font-bold ${isMe ? 'self-end' : 'self-start'}`}>
                ...
                {currentChatMessages.length === 0 && (
                  <div className="flex-1 flex items-center justify-center py-20 text-center">
                    <div>
                      <Sparkles className="text-sc-accent/30 mx-auto mb-3" size={32} />
                      <p className="text-sc-muted text-sm">Start the transmission with <strong>{activeContact.displayName}</strong></p>
                    </div>
                  </div>
                )}
              ...
              <div className="flex-shrink-0 p-4 bg-[#081329]/80 backdrop-blur-md border-t border-white/5">
                <form onSubmit={handleSend} className="relative flex items-center">
                  ...
                  >
                    {isSending ? <Loader size={15} className="animate-spin" /> : <Send size={15} className="relative left-[1px]" />}
                  </button>
                </form>
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-sc-muted text-sm max-w-sm mb-6">Select a connection or click the pencil icon to open a new neural channel.</p>
              <button
                onClick={() => setShowNewSynapse(true)}
