    ...
    if client:
        try:
            ...
            if rows:
                ...
                for r in rows:
                    r["engagement_summary"] = "Trending" if r.get("likesCount", 0) > 50 else "Active"
                ...
                for idx, r in enumerate(rows):
                    ...
                    passage_text = f"User: @{user} | Emotion: {emotion} | Context: {caption}"
            ...
            if rows:
                ...
                try:
                    ...
                    for i, item in enumerate(imagined[:3]):
                        ...
                        photo_id = MOOD_IMAGE_MAPPING.get(mood, "photo-1493612276216-ee3925520721")
            ...
            return SearchResponse(posts=final_rows, query=req.query, total=len(final_rows), mode="semantic-reranked" if rows else "synthetic-discovery")
        except Exception as e:
            pass
